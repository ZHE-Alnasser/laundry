<div x-data="pos" class="bg-gray-50 sm:m-4">
    <div class="sm:flex">
                    ...
                    <div class="modal-box">
                        ...
                        <div class="modal-action">
                            <label for="services-modal">
                                ...
                                <span class="btn btn-primary"><?php echo e(__('close')); ?></span>
                            </label>
                        </div>
                    </div>
                </div>
        ...
        <div class="order">
            <template x-for="(o,index) in order" :key="index">
                <div
                    class="mx-10 bg-white border-b-2 border-blue-200 border-none f sm:flex items-center justify-around justify-evenly m-2 p-4 rounded hover:bg-blue-100 text-black">
                    <div>
                        <div x-text="o.item_name"></div>
                        <div class="text-gray-600" x-text="o.name"></div>
                        <div class="ms-2 text-gray-700"
                             x-text="'<?php echo e(__('Unit Price')); ?>: ' + o.price + '<?php echo e(currency()); ?>'"></div>
                    </div>
                    ...
                    <div class="mx-3 mt-3" x-text="o.total"></div>
    ...
    <div class="flex items-end my-4">
        ...
        <input type="number" step="0.1" autocomplete="off" min="total" name="paid" class="input flex-1"
               x-model="paid" label="<?php echo e(__('Paid Amount')); ?>"
               x-on:change="calculatePayments($event.target.value)"
               x-on:keydown.debounce.150ms="calculatePayments($event.target.value)"/>

        <button  type="button" class="h-full btn btn-primary" x-on:click="calculatePayments(total)"><?php echo e(__('Full Amount')); ?></button>
    </div>
    ...
    <div class="flex justify-between text-black">
        <div class="sm:flex">
            ...
            <h4 class="mx-2 my-2" x-text="'<?php echo e(__('Total')); ?>:' + total"></h4>
            <input  name="change" x-bind:value="total" hidden />
            <h4 class="mx-2 my-2" x-text="'<?php echo e(__('Change')); ?>:' + change"></h4>

          
            <input  name="vat" x-bind:value="total * vat/100" hidden />
            <h4 class="mx-2 my-2" x-text="'<?php echo e(__('Total VAT')); ?>:' + total * vat/100"></h4>

            <input  name="total"  x-model="totalWithVat" x-bind:value="total + total * vat/100" hidden />
            <h4 class="mx-2 my-2" x-text="'<?php echo e(__('Total amount with VAT')); ?>:' + (total * vat/100 + total)"></h4>
...
<script>
console.log(<?php echo json_encode($items->load('services'), 15, 512) ?>);
    ...
    document.addEventListener('alpine:init', () => {
        Alpine.data('pos', () => ({
            ...
            index: 1,
            items: <?php echo json_encode($items->load('services'), 15, 512) ?>,
            currentItem: null,
            totalWithVat:0,
            services: [

            ],
            vat: <?php echo json_encode(setting('vat_rate'), 15, 512) ?>,
            total: 0,
            totalVat:0,
            paid: 0,
            change: 0,
...
<?php /**PATH C:\xampp\htdocs\laundry\resources\views/orders/services/pos.blade.php ENDPATH**/ ?>